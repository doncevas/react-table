...
import { getProducts, createProduct, removeProduct } from '../../api/product';
...
export function* addProduct(action) {
  try {
    const result: any = yield call(createProduct, action.payload);
...
export function* removeProductItem(action) {
  try {
    const result: any = yield call(removeProduct, action.payload);
